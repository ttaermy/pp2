...
def search_contacts(pattern):
    """Search contacts by pattern (name, surname, or phone)"""
    conn = sqlite3.connect("phonebook.db")
    cursor = conn.cursor()
...
def insert_many_contacts(contacts):
    ...
    invalid_contacts = []
    phone_pattern = re.compile(r'^[0-9]{10,15}$')  # Simple phone validation
    ...
    for name, phone in contacts:
        if phone_pattern.match(phone):
            try:
                ...
                invalid_contacts.append((name, phone))
        else:
            invalid_contacts.append((name, phone))
...
def get_contacts_paginated(limit, offset):
    ...
    cursor.execute("SELECT * FROM PhoneBook ORDER BY name LIMIT ? OFFSET ?", (limit, offset))
    results = cursor.fetchall()
    conn.close()
    return results
